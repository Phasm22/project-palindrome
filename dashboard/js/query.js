...
export async function executeQuery() {
  ...
  try {
    ...
    let html = '<h3 style="color: #60a5fa; margin-bottom: 15px;">Answer</h3>';
    html += `<div style="color: #e2e8f0; margin-bottom: 20px; padding: 15px; background: #1e293b; border-radius: 4px;">${data.answer || 'No answer provided'}</div>`;

    if (data.sources && data.sources.length > 0) {
      html += '<h3 style="color: #60a5fa; margin-bottom: 15px;">Sources</h3>';
      html += '<table><thead><tr><th>Source</th><th>Score</th><th>Preview</th></tr></thead><tbody>';
      data.sources.forEach(source => {
        html += `<tr>
          <td>${source.sourcePath || source.chunkId || 'Unknown'}</td>
          <td>${(source.score || 0).toFixed(4)}</td>
          <td>${(source.text || '').substring(0, 100)}...</td>
        </tr>`;
      });
      html += '</tbody></table>';
    }
...
export async function executeGraphQuery() {
  ...
  try {
    ...
    let html = '<h3 style="color: #60a5fa; margin-bottom: 15px;">Graph Query Results</h3>';
...
export async function executeCypherQuery() {
  ...
  try {
    ...
    let html = '<h3 style="color: #60a5fa; margin-bottom: 15px;">Cypher Query Results</h3>';
    html += `<div style="color: #94a3b8; margin-bottom: 15px;">Found ${data.nodes?.length || 0} nodes, ${data.relationships?.length || 0} relationships</div>`;
